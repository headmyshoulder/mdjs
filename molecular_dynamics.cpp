...
namespace js
{
    extern "C" 
    {
        ...
        extern void init_point(int id, int window_id);
        extern void move_point(int id, int x, int y, int time);
        extern int get_cycel_time();
...
{  
    ...
    const int point_id_base=window_id;
    ...
    for(size_t i = 0; i < n; ++i)
    {
      js::init_point(point_id_base+i, window_id);
    }
    ...
    for( size_t oi=0 ; oi<500 ; ++oi )
    {
        ...
        for( size_t i=0 ; i<n ; ++i )
        {
            ...
            js::move_point(point_id_base+i, x[i], x[i+n], 50*oi+5000);
        }
        //std::cout << "e" << std::endl;
        //std::chrono::milliseconds dura( js::get_cycel_time() );
        //std::this_thread::sleep_for( dura );
        //js::sleep(js::get_cycel_time());
    }
